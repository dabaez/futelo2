...
describe('buyPickaxe', () => {
  test('throws when the user has fewer coins than PICKAXE_COST', () => {
    ...
    requireRoomMember.mockReturnValue(makeUser({ coins: PICKAXE_COST - 1 }));
  ...
  test('deducts PICKAXE_COST from coins', () => {
    ...
    expect(stmts.updateRoomCoins.run).toHaveBeenCalledWith(-PICKAXE_COST, 0, 1);
  });
  ...
  test('wraps all DB writes in a single transaction', () => {
    ...
    requireRoomMember.mockReturnValue(makeUser({ coins: 100 }));
    stmts.getRoomMember.get.mockReturnValue(
